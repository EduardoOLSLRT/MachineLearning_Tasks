# --- Missão 5: Modelos de Machine Learning - Regressão e Classificação ---
...
# ====================================================================
# EXERCÍCIO 1: Prevendo Preços de Imóveis (0,10pt)
# ====================================================================
...
# Comentários: Nesta primeira tarefa montamos um modelo simples de Regressão Linear
# para prever o preço de imóveis. O fluxo é:
#  - X_treino_final: matriz de features já pré-processadas (numéricas padronizadas + one-hot das categorias)
#  - y_treino: alvo (preços) usado para ajustar os coeficientes do modelo
#  - Depois de treinar (.fit), fazemos previsões em X_teste_final e calculamos métricas (MAE, R2)
#  Observação: garantir que X_treino_final e X_teste_final estejam alinhados (mesmo número de colunas) é essencial.
modelo_preco_casas = LinearRegression()

# 2. Treine o modelo com os dados de TREINO. Use o método .fit() com os dados de treino (X_treino_final e y_treino).
modelo_preco_casas.fit(X_treino_final, y_treino)
print("Modelo treinado com sucesso!")

# 3. Faça previsões nos dados de TESTE. Use o método .predict() no conjunto de teste (X_teste_final).
previsoes = modelo_preco_casas.predict(X_teste_final)

print("\n--- Resultados da Avaliação ---")
# Vamos comparar as previsões com os valores reais (y_teste)
for i in range(len(previsoes)):
    # Aqui mostramos cada previsão lado a lado com o valor real para entender discrepâncias
...
# ==================================================================
# EXERCÍCIO 2: Regressão - Previsão de Preço de Carros Usados (0,10pt)
# ==================================================================
...
X = df_carros.drop('Preco', axis=1) # Remove a coluna do data frame
y = df_carros['Preco']
...
# Pré-processamento (Exercício 2 - Carros):
# - Para as colunas numéricas aplicamos imputação (mediana) e padronização
#   com StandardScaler. Isso cuida de valores faltantes e coloca as features
#   em uma escala comparável (importante para muitos modelos).
# - Para as colunas categóricas aplicamos OneHotEncoder para transformar
#   categorias em colunas binárias. Usamos handle_unknown='ignore' para
#   evitar erros quando o conjunto de teste contiver categorias novas.
...
# ==================================================================
# EXERCÍCIO 3: Classificação - Detecção de Churn de Clientes (0,10 pontos)
# ==================================================================
...
# Comentários (Exercício 3 - Churn):
# - O pipeline numérico usa imputação por média e padronização para garantir
#   que valores faltantes sejam tratados e que features numéricas fiquem
#   na mesma escala.
# - O pipeline categórico transforma variáveis categóricas em indicadores
#   binários (one-hot). O ColumnTransformer garante que cada tipo de
#   transformação seja aplicado apenas às suas colunas correspondentes.
# - Ao incluir este `preprocessor` dentro do Pipeline final com o
#   classificador, chamando `pipeline_final_cla.fit(X_treino, y_treino)`:
#   o preprocessor é ajustado apenas no conjunto de treino e depois usado
#   para transformar treino e teste antes do treinamento/avaliação do modelo.
